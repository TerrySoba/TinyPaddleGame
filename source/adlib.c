...
void WriteFM(int reg, int value)
{
    ...
    for (i = 0; i < 6; i++)
        inp(ADLIB_FM_ADDRESS);        /* wait 12 cycles       */
    outp(ADLIB_FM_DATA, (BYTE)value); /* write out the value  */
    for (i = 0; i < 35; i++)
        inp(ADLIB_FM_ADDRESS); /* wait 84 cycles       */
} /* End of WriteFM */
...
void FMReset(void)
{
    ...
    /* zero all registers */
    for (i = MIN_REGISTER; i < MAX_REGISTER + 1; i++)
        WriteFM(i, 0);
...
// void printInstrument(FMInstrument *ins)
// {
//     printf("{\n");
//     for (int i = 0; i < sizeof(FMInstrument); ++i)
//     {
//         printf("0x%02x, ", ((unsigned char*)ins)[i]);
//     }
//     printf("}\n");
// }
